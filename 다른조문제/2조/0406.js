...
const adventureMovies = movies.filter((movie) =>
  movie.장르.includes("어드벤쳐")
);
const adventureMovieTitles = adventureMovies.map((movie) => movie.제목);
console.log(adventureMovieTitles);
// }
// // {
// // 2. 신카이 마코토 감독이 만든 영화들의 장르를 보여주세요.
const director = movies.filter((movies) =>
  movies.감독.includes("신카이 마코토")
);
const movieDirector = director.map((movie) => movie.장르);
console.log(movieDirector);
// // }
// // {
// // 3. 개봉년도가 3번째인 영화의 정보를 모두 출력해주세요
const sortedMovies = movies.sort((a, b) => a.개봉년도 - b.개봉년도);

console.log(sortedMovies[2]);
